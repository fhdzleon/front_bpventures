...
const ListInvoiceComponent = ({
  ...
  const hasPermission = (invoice: { permissions: any[] }, permission: any) => {
    return (
      invoice.permissions &&
      Array.isArray(invoice.permissions) &&
      invoice.permissions.some((p) => p.permissionType.name === permission)
    );
  };
        ...
        <table className="min-w-full divide-y divide-gray-300">
          <thead className="bg-secundary font-futura text-white">
            <tr>
              ...
              <th className="py-3 px-4 font-futura text-center text-lg">
                Monto
              </th>
              <th className="py-3 px-4 font-futura text-center text-lg">
                Estado
              ...
              <th
                className={`py-3 px-4 font-futura text-center text-lg ${
                  userData?.isAdmin ? "" : "bg-secondary text-transparent"
                }`}
              >
                {userData?.isAdmin ? "Permisos" : ""}
              </th>
            </tr>
          </thead>
            ...
            {currentInvoices.length > 0 ? (
              currentInvoices.map((invoice: InvoiceInterface) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 font-sans text-sm text-center text-gray-700">
                    ${invoice.amount}
                  </td>
                  <td
                    className={`py-4 px-6 font-sans text-sm text-center ${getStatusColor(
                  ...
                  <td className="py-4 px-6 font-sans text-sm text-center justify-center text-gray-700 flex space-x-4">
                    <InvoiceDownload
                      userId={userData?.id || ""}
                    ...
                    {userData?.isAdmin ? (
                      ...
                      invoice.permissions?.some(
                        (p: { permissionType: { name: string } }) =>
                          p.permissionType.name === "view"
                      ) && (
                        <button
                          title="vista previa"
                          onClick={() => handleOpenModal(invoice)}
                    ...
                    {isModalOpen && selectedInvoice && (
                      <div
                        className="fixed inset-0 flex items-center justify-center bg-secundary z-50"
                      ...
                      >
                        ...
                        >
                          <button
                            onClick={handleCloseModal}
                            className="mt-4 bg-secundary text-white px-4 py-2 rounded-full flex ml-auto"
                          >
                            Cerrar
                          </button>
                          <InvoiceDetail
                    ...
                    {isModalOpenVoucher && selectedInvoice && (
                      <div
                        className="fixed inset-0 flex items-center justify-center bg-secundary ml-0 z-50"
